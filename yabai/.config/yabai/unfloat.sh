...
yabai -m query --windows | jq -c '.[]' | while read -r window; do
	...
	# If the window belongs to one of the target apps, is floating, and the title is not "window-switcher", unfloat it
	if [[ "$is_floating" == "true" && " ${apps_to_unfloat[*]} " == *" $app_name "* && "$window_title" != "window-switcher" ]]; then
